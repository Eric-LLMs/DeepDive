...
| Observability | a best-effort JSONL audit trail (`data/audit.jsonl` — agent tool audit + worker events) and relational usage surfaces (`user_usage_logs`, `wallet_transactions`, `session_events` with retention, `workspace_activity`, `jobs` status) exist, but the unified PostgreSQL log tables (audit_logs / ai_call_logs / activity_logs / job_logs) were never created |
| Video → searchable chunks | video currently produces a PPT/PDF *book* (`generate_media`); the designed media → timestamped-chunks **ingestion into RAG search** is not implemented (document ingestion is — see Implemented) |
