...
| Retrieval-feedback loop | grounded answers rate in place: the turn's `rag_search` hits are extracted server-side from the tool trace (`_extract_retrieval`), snapshotted into `messages.meta.retrieval` (JSONB, migration 0016) and returned in the done frame / `GET /sessions/{id}`; the chat bubble grows a 👍/👎 panel (optional reason) that posts the existing `POST /rag/feedback` golden-set recorder — persistence survives reopen, no per-hit re-query needed |
| Video → searchable chunks | subtitle assets (`.srt` / `.vtt` / `.lrc`) ingest as **cue-grouped timestamped chunks** instead of flattened prose (`build_subtitle_chunks`): consecutive cues merge up to `chunking.chunk_chars`, each chunk carries `meta {kind:"subtitle", video, start_ms, end_ms, start_ts, end_ts}` with the video named by the subtitle file's stem — `meta` rides recall → `rag_search` results verbatim, so the agent cites `<video> @ H:MM:SS` per hit |
