...
- **Capture & attach a region**: the chat input's 📷 button freezes the current window (`webContents.capturePage`), then you drag to select a region (double-click = whole frame) before it's uploaded as a real-MIME PNG to the `chat/temp/` cloud folder and staged as the pending attachment; the screenshot renders **inline as a thumbnail** in the chat bubble instead of a filename chip. A screenshot is **temporary** — it lives in the `chat/temp/` scratch zone the user may empty at will, and deleting its message/session trashes it (recoverable, folder-agnostic — the delete keys off the message's `attach_asset_id`, not the folder) — **except** a Q&A pair imported into RAG, whose image is **moved to `RAG/images/`** (out of the clearable zone) and preserved with the corpus instead of deleted — [design: §18.2](architecture.md#182-chat-screenshot-pipeline).
...
- **Chat screenshots imported into RAG are preserved**: a chat Q&A whose question carried a screenshot writes that asset into the chunk's `meta.image_ids` on import (single-pair and whole-session imports alike), so retrieval returns it for the `vision` tool. The import also **moves** the image out of the temporary `chat/temp/` scratch zone into `RAG/images/` — so clearing `chat/temp/` can't orphan a referenced image — and the `imported_rag` message flag **gates the session/message delete cascade**. A screenshot is temporary and dies with its chat; once the Q&A joins the query repository the image is stored alongside the RAG text and survives deletion — [design: §18.2](architecture.md#182-chat-screenshot-pipeline).
