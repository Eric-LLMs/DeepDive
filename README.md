...
- **Governed research workflows (Research OS).** A ten-phase research DAG runs under hard gates, human review, and an evidence-trace graph, driven by one of two creation-locked execution modes: **strict** — an unmet gate halts the transition until human review or repair — and **progressive** — an unmet gate is recorded as a diagnostic gap and the run auto-settles forward, so it can still reach PUBLISH with its unverified items honestly disclosed, never deadlocking and never papering over gaps. Evidence gathering follows a **wholesale batch pipeline**: server-side concurrent scraping behind SSRF guardrails feeds an immutable fetch-provenance ledger, node verification is anchored to that ledger rather than to model assertions, and the whole evidence graph is surfaced in a reactive desktop view. A single chat turn atomically scaffolds the task and its mirrored cloud-drive directory (materials/outputs/temp/spec/session-history), rolling back entirely on failure. Execution is server-owned and auto-driven to PUBLISH via chained background arq workers (25 steps each vs interactive 5, bounded by turn / no-progress / attempts / cost caps). Tool-boundary schema pre-decoding transparently repairs LLM-stringified JSON payloads, while a RUNNING badge and `research_continuing` keep client state strictly aligned without active connections. State persistence follows a single-writer cross-process CAS via portalocker on monotonic `project_revision` (tmp → fsync → replace). Stalled heartbeats recover idempotently on the same turn, adopting stale slots to eliminate wedges and double runs. Workflows remain strictly controllable: cooperative cancellation (cancel flag, never a mid-write kill) exits cleanly, and gate failures park in BLOCKED behind durable PENDING overrides until human review atomically flips them to OVERRIDE to resume, or rejects so the agent proposes another approach. A lightweight SSE channel broadcasts monotonic revision hints paired with coalesced client refetches (~300ms) to eliminate UI flickering and request storms, complemented by per-role rotating logs stamped with request/user/session/task/run context. Cascade-safe deletion blocks active or KB-indexed tasks (409), trashes storage directories, and deletes the task's dedicated chat sessions with it, without resurrection risks.
