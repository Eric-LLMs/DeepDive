...
- **Configurable retrieval.** A modular node pipeline — vector + keyword recall, RRF fusion, cross-encoder rerank, parent expansion, and CRAG relevance checks — can be reconfigured, reordered, or toggled live via the admin console without service restarts. It includes chunking previews, golden-set evaluation (`Recall@k`, `Precision@k`, `MRR`), Redis query caching (keyed by query + config + corpus version, auto-invalidated on re-index), and vision-LLM transcription for PDF tables. A single node failing degrades to the surviving channels instead of breaking the chat, and user feedback is logged to a golden evaluation dataset.
