...
- **Governed research workflows (Research OS).** A ten-phase research DAG runs under hard gates, human review, and an evidence-trace graph. A chat turn atomically creates a task — the cloud-drive task folder (materials/outputs/spec/session-history) mirrors a scratch authority, rolled back on failure. Runs are server-owned: disconnecting never cancels the work, a RUNNING badge tracks progress, a per-task mutex rejects duplicate triggers with HTTP 409, and stale runs from a crash window are adopted and cleaned up. Deleting a task is cascade-safe: blocked with 409 while running or indexed into the Knowledge Base, the cloud folder goes to Trash, and restoring it never revives task state.
