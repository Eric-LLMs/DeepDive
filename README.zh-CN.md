...
* **材料是探索的起点，而非认知的边界**：当现有资料不足时，导师能够自主联网检索学术文献与外部数据，拓展认知深度。
* **学习成果不会丢失**：核心认知沉淀为持久记忆，讨论过程自动转化为摘要、思维导图与演示幻灯片，回流至可检索工作区，支持跨设备随时无缝续学。

> **知识闭环流转：** `导入资料 → 智能索引 → 精准召回 → 深度推理与转化 → 沉淀为知识产物 → 重新被索引检索`
...
| 能力模块 | 具体应用场景 |
| :--- | :--- |
| **学习与理解** | • 边读边看边提问 —— 支持 PDF、Office 文档、视频、音频、图片等多格式直读<br>• 获得分步推理讲解与核心概念拆解<br>• 针对选中的段落、页面或视频片段随时展开深入探讨 |
| **研究与探索** | • 单次查询即可跨越个人文件、笔记、历史会话与导入资料库进行全域检索<br>• 本地材料不足时自主联网检索补充<br>• 将多源材料综合提炼为有据可查的结构化答案 |
...
* **持久记忆与会话历史彻底解耦**：两条独立轨道共享统一的 Prompt 上下文边界——Agent 主动写入长期文件记忆，系统在 PostgreSQL 中自动维护情景会话记忆（通过 RRF 算法融合 `tsvector` 全文检索与 `pgvector` 向量检索，并引入时间衰减权重）。分层历史压缩（扁平 Token 窗口）在保证核心上下文不丢失的同时，将上下文严格限制在安全窗口内。
* **可靠性原生内嵌，拒绝事后补丁**：针对上游 LLM 的瞬态错误，内置硬超时与指数退避重试机制，并通过单轮成本预算设置硬顶限额。工具安全由多重防线共同保障：基于 Redis Pub/Sub 的人工审批门（HITL，超时默认拒绝）、Plan 规划模式、有界子代理、用于状态回滚的 Shadow-Git 检查点，以及具备网络隔离和资源配额的 Docker 沙箱。
* **检索流程高度可配置，无需硬编码**：模块化节点管道（查询重写 → 向量与关键词召回 → RRF 融合 → Cross-Encoder 重排 → 父块扩展 → CRAG 相关性校验）支持在管理后台实时调整拓扑、重排顺序或启闭节点，无需重启服务。内置分块预览、Golden-set 黄金测试集评测（Recall@k、Precision@k、MRR）、基于版本感知的 Redis 查询缓存（按 query + config + corpus version 组合为 Key，重新索引自动失效）以及基于视觉大模型的 PDF 表格解析。单节点故障时自动降级至可用通道，保障对话不中断，同时用户反馈会被实时记录并沉淀至评测数据集。
...
* **统一知识底座，告别孤立存储**：个人网盘与共享团队工作区（具备 Owner / Admin / Editor / Viewer 角色权限、成员管理与追加式审计日志）基于 SHA-256 内容寻址对象存储构建，支持引用计数去重、8 MB 断点分块续传、多级目录、30 天回收站留存、文件级 ACL、公开分享链接与多格式在线预览。网盘同时作为内容处理、检索与 Agent 工作流的共享工作目录。
* **权限控制在资源边界处强制执行**：角色权限、租户边界、上游 LLM 凭证、模型目录与路由权重均由统一管理后台集中管控。提供带掩码（`sk-***`）的每用户密钥授权矩阵、用于邮件验证 / 密码重置的 SMTP 服务，以及无状态签名 Admin 会话机制。
* **Local-First 客户端配合私有化部署**：Electron 工作台支持离线文件工作流（文件树浏览、多格式查看器、视频逐帧截图）；大体积媒体在客户端本地预处理并回传分析产物，常规计算任务由服务端承载。完整后端技术栈（PostgreSQL/pgvector、Redis、TEI 向量推理、Kokoro TTS 与 LiteLLM 网关）支持通过 `docker-compose` 一键拉起，确保所有数据完全留存在你自己的基础设施之内。
