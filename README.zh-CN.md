...
* **Research OS：有状态的受控研究工作流**：将 AI 深度研究拆解为十阶段 DAG，由硬性质量门禁、人工审批和证据溯源图协同推进。单次对话即可原子化创建完整项目，并在网盘同步生成包含参考材料、输出产物、任务配置与会话记录的专属目录，创建失败全量回滚。任务执行由服务端原生托管，支持离线全流程自动推进：对话交接后由后台异步工作链（arq）接力自驱至最终发布阶段（PUBLISH），内置轮次上限、无进展截断与成本熔断保护，并在工具调度层支持参数自动预解码以防御大模型 JSON 序列化幻觉；任务运行态贯穿整条后台链路，前端状态实时对齐。状态写入统一采用跨进程单写者 CAS 机制：基于文件锁与单调递增版本号，严格按“临时文件 → 强制刷盘（fsync） → 原子替换”持久化；具备完善的抗崩溃自愈能力，心跳超时后同轮次幂等接管重跑，自动回收死槽，杜绝重复执行与死锁。运行全程可控，集成人机协同与轻量化监控：支持协作式安全停止（设置取消标记优雅退出，不掐断写盘）；遇到质检门禁未通过时任务自动安全挂起，支持人工审批放行（原子标记覆盖并唤醒续跑）或驳回重试；实时监控仅通过 SSE 广播版本变更信号，配合客户端防抖合并机制按需拉取以彻底杜绝界面乱序闪烁与请求风暴，全链路日志自动注入统一上下文追踪标记。删除动作天然级联安全：运行中或已入库的内容一律拦截（409），网盘目录移入回收站，即便手动还原也绝不会让已注销的任务“死而复生”。
