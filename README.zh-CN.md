...
* **Research OS：有状态的受控研究工作流**：将 AI 深度研究拆解为十阶段 DAG，由硬性质量门禁、人工审批和证据溯源图协同推进。单次对话即可原子化创建完整项目，并在网盘同步生成包含参考材料、输出产物、任务配置与会话记录的专属目录，创建失败全量回滚。任务执行由服务端原生托管，前台断开或切出页面不中断后台计算，配合状态徽标跟踪、防重复触发的 409 互斥锁，以及异常中断后的自动接管机制。删除动作天然级联安全：运行中或已入库的内容一律拦截（409），网盘目录移入回收站，即便手动还原也绝不会让已注销的任务“死而复生”。
