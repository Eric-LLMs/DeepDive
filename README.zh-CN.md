...
* **Research OS：有状态的受控研究工作流**：将 AI 深度研究拆解为十阶段 DAG，由硬性质量门禁、人工审批和证据溯源图协同推进。单次对话即可原子化创建完整项目，并在网盘同步生成包含参考材料、输出产物、任务配置与会话记录的专属目录，创建失败全量回滚。任务执行由服务端原生托管，并**由后台工作链自动推进直至 PUBLISH**：聊天交接后由 arq 任务逐轮接管（每轮 25 步 vs 交互式 5 步，受总轮数 / 无进展轮 / 重试次数 / 成本上限约束），一次提问即可在无客户端挂接的情况下完成整套受控调研，`RUNNING` 状态徽标与 `research_continuing` 贯穿整条后台链直至终局。任务状态写入统一走**跨进程单写者比较-交换（CAS）**：portalocker 锁 + 单调递增的 `project_revision`，按 `临时文件 → fsync → 原子替换`持久化；运行天然抗崩溃——心跳超时后**在同一轮次幂等重跑**、陈旧运行槽自动接管，杜绝重复执行或任务被永久卡死。运行全程可控：支持**协作式停止**（仅置取消标记、绝不在写入中途强杀）与**人工门禁审批**——Agent 无法推进某道 Gate 时，任务以 BLOCKED 状态挂起在持久化的 PENDING 审批单之后，待人审批通过（门禁翻转为 OVERRIDE、运行自动恢复）或驳回（Agent 另寻思路重新提议）；SSE 实时监控只推送版本变更提示（每次只触发一次合并后的任务刷新）而非负载，服务端进程另按角色输出滚动日志，并携带 request/user/session/task/run 上下文标记。删除动作天然级联安全：运行中或已入库的内容一律拦截（409），网盘目录移入回收站，即便手动还原也绝不会让已注销的任务“死而复生”。
