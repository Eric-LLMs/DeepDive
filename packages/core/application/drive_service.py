...
from core.infrastructure.db import AssetModel
...
class DriveService:
    ...
    async def save_artifact(
        ...
        *,
        ...
        """Persist server-generated bytes as a READY asset in the user's drive.

        Used by the toolkit worker to drop a generated artifact (mindmap / slides / summary)
        into a chosen folder (default = the user's drive root). The asset name goes through
        :meth:`_unique_name` so a re-run auto-suffixes instead of colliding. ``source_asset_id``
        binds a RAG-derived image to its source document so re-ingest dedupes and source
        deletion cascades.
        """
        name = await self._unique_name(user_id, workspace_id, folder_path, name)
        ...
        await self.objects.upsert_and_increment(digest, len(content), storage_key, mime_type)
    ...
    async def delete_asset(self, user_id: UUID, asset_id: UUID) -> dict:
        ...
        retention sweep, so a trashed file can still be restored. Deleting a RAG source
        document cascades the soft-delete to its derived image assets so they trash together.
        """
        asset = await self.ensure_asset_writable(user_id, asset_id)
        deleted = await self.assets.soft_delete(asset.id)
        if deleted is None:
            raise DriveError("asset not found", 404)
        for derived in await self.assets.list_by_source(asset.id):
            await self.assets.soft_delete(derived.id)
        await self._log(
    ...
    async def _purge_asset(self, asset) -> dict:
        ...
        Derived RAG images (``source_asset_id``) are purged FIRST so each decrements its own
        refcount before the source row drops; the DB ``ON DELETE CASCADE`` on
        ``source_asset_id`` is a safety net that then never fires on the app path.
        """
        for derived in await self.assets.list_by_source(asset.id, include_deleted=True):
            await self._purge_asset(derived)
    ...
    async def restore_trash(self, user_id: UUID, asset_id: UUID) -> dict:
        ...
        # Derived RAG images trashed with the source come back with it.
        for derived in await self.assets.list_by_source(asset.id):
            await self.assets.restore(derived.id)
        await self._log(
