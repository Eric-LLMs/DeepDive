...
async def test_unchanged_snapshot_reuses_head(tmp_path):
    ...
    c1 = await store.snapshot("first")
    c2 = await store.snapshot("second")  # nothing changed
    assert c1 == c2
...
async def test_revert_unknown_id_raises(tmp_path):
    ...
    (ws / "a.txt").write_text("x")
    store = CheckpointStore(ws, tmp_path / "shadow")
    await store.snapshot("baseline")

    with pytest.raises(CheckpointError):
        await store.revert("deadbeefcafe")
